    ...
    try {
      try {
        ...
        if (response.accessToken) {
          // Store token (best effort - may fail silently if service worker not ready)
          // Token is in memory and will work for this session regardless of storage
          try {
            ...
            // Storage failure is non-critical - token is in memory
            console.warn('Token storage failed (non-critical):', storageError);
          }

          // Continue with login - token is available in memory
          // The axios interceptor will use the token from memory even if storage failed
          ...
          // No access token in response - this is a real error
          console.error('Login response missing accessToken');
          showErrorToast('Login failed: No access token received');
          handleFailedLogin();
        }
